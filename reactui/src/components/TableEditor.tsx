...
type Props = {
  ...
  rows: unknown[][];                // from query result
...
export default function TableEditor({ fullName, rows, columns, onStatus }: Props) {
  ...
  useEffect(() => {
    const objRows = rows.map((r) => {
      ...
      columns.forEach((c, idx) => (o[c] = r[idx] as string | number | boolean | null));
      return o;
    });
    setData(objRows);
  }, [rows, columns]);
  ...
  function getPkForRow(rowObj: RowData): Record<string, string | number | boolean | null> | null {
  ...
  async function commitCell(rowIndex: number, colName: string, newValue: string | number | boolean | null) {
    ...
    return columns.map((colName) => {
      ...
      return {
        ...
        cell: (ctx) => {
          ...
          const value = ctx.getValue<string | number | boolean | null>();
          ...
          if (isDateType(t)) {
            const iso = value && typeof value !== "boolean" ? new Date(value).toISOString().slice(0, 10) : "";
            return (
              <input
                type="date"
